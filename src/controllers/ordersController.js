...
async function showOrders(req, res) {
    ...
    try {
        ...
        if(date) {
            ...
            requests.rows = requests.rows.filter(time => time.createdAt.getDate() == dayFormat.getDate() &&
                time.createdAt.getMonth() == dayFormat.getMonth() &&     
                time.createdAt.getFullYear() == dayFormat.getFullYear()
            );
        }
